...
<template>
  <TutorialPage title="部署到 Cloudflare" description="Cloudflare Pages 是目前部署 Nuxt 應用程式的最佳選擇之一。它提供了全球 CDN 加速、免費的 HTTPS 與極致的 Edge 運算效能。"
    ...
    <TutorialSection id="configuration" title="專案設定" icon="heroicons:cog-6-tooth" separator>
      <p class="text-slate-400 mb-6">
        雖然 Cloudflare 通常能自動偵測 Nuxt，但為了確保最佳相容性，我們建議在 <code class="text-slate-300">nuxt.config.ts</code> 中明確指定 Preset。
      </p>
  ...
  nitro: {
    ...
    preset: 'cloudflare-pages'
      ...
      <div class="mt-6 p-4 rounded-xl bg-slate-900/50 border border-slate-800/50">
        ...
        <p class="text-slate-400 text-sm">
          請確保您的專案在 Cloudflare 建置環境中使用 <code class="text-emerald-400">Node.js 18+</code> 或 <code class="text-emerald-400">20+</code>。
      ...
      <div class="space-y-6">
        <div class="p-6 rounded-xl bg-slate-950 border border-slate-800/50">
          <h4 class="text-white font-bold mb-4 flex items-center gap-2">
            <UIcon name="heroicons:adjustments-horizontal" class="w-5 h-5 text-emerald-400" />
            Build Settings (關鍵設定)
          </h4>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="p-4 rounded-lg bg-slate-900/50 border border-slate-800">
              ...
              <div class="text-emerald-400 font-mono font-bold">pnpm build</div>
              <div class="text-xs text-slate-500 mt-1">或 npm run build</div>
            </div>
            <div class="p-4 rounded-lg bg-slate-900/50 border border-slate-800 ring-1 ring-yellow-500/50">
              ...
              <div class="text-yellow-400 font-mono font-bold">.output/public</div>
              <div class="text-xs text-yellow-500/80 mt-1">⚠️ 預設 dist 是錯的，請務必修改！</div>
            </div>
          </div>
        </div>
        ...
        <ol class="space-y-4 text-slate-400 list-decimal list-inside marker:text-emerald-500">
          ...
          <li class="pl-2">登入 <a href="https://dash.cloudflare.com/" target="_blank" class="text-emerald-400 hover:underline">Cloudflare Dashboard</a>，進入 <strong>Workers & Pages</strong>。</li>
          <li class="pl-2">點擊 <strong>Create application</strong> > <strong>Pages</strong> > <strong>Connect to Git</strong>。</li>
      ...
      <div class="space-y-4">
        <div class="p-4 rounded-xl bg-slate-900/50 border border-slate-800/50">
          <h4 class="text-white font-medium mb-2 flex items-center gap-2">
            <UIcon name="heroicons:document" class="w-5 h-5 text-red-400" />
          ...
          <ul class="list-disc list-inside text-sm text-slate-400 ml-2">
            <li>Nuxt 2 或靜態生成預設為 <code class="text-slate-300">dist</code>。</li>
            <li>Nuxt 4 (Nitro) 預設輸出為 <code class="text-slate-300">.output/public</code>。</li>
        ...
        <div class="p-4 rounded-xl bg-slate-900/50 border border-slate-800/50">
          <h4 class="text-white font-medium mb-2 flex items-center gap-2">
            <UIcon name="heroicons:server" class="w-5 h-5 text-yellow-400" />
            500 Server Error
          </h4>
          <p class="text-sm text-slate-400 mb-2">
          ...
          <ul class="list-disc list-inside text-sm text-slate-400 ml-2">
            <li>是否在 <code class="text-slate-300">nuxt.config.ts</code> 中設定了 <code class="text-slate-300">nitro.preset: 'cloudflare-pages'</code>。</li>
