...
<template>
    <DocsPage title="TypeScript 支援"
        ...
        <!-- Section 1: Why TS -->
        <DocsSection id="why-ts" title="為什麼選擇 TypeScript？" icon="logos:typescript-icon" separator>
        ...
        <DocsSection id="type-check" title="型別檢查" icon="heroicons:check-badge" separator>
            <p class="text-slate-400 mb-6">
                Nuxt 的開發伺服器預設不會執行型別檢查 (為了速度)。要啟用即時型別檢查或在建置時檢查，請安裝 <code class="text-emerald-400">vue-tsc</code>。
            </p>

            <UiCodeBlock language="bash" :code="`pnpm add -D vue-tsc typescript`" filename="Terminal" />
  ...
  typescript: {
    ...
    strict: true
        ...
        <DocsSection id="api-typing" title="API 型別安全" icon="heroicons:server-stack" separator>
            <p class="text-slate-400 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <UiCodeBlock language="typescript" :code="`export default defineEventHandler(() => {
  return {
    id: 1,
    name: 'Zayn',
    role: 'admin' as const
                ...
                <div>
                    ...
                    <UiCodeBlock language="typescript" :code="`const { data } = await useFetch('/api/user')
...
console.log(data.value?.name)`" filename="Client" />
        ...
        <DocsSection id="component-props" title="元件 Props" icon="heroicons:code-bracket" separator>
            <p class="text-slate-400 mb-6">
                使用 <code class="text-emerald-400">defineProps</code> 搭配 TypeScript 介面，可以獲得完美的 Props 型別提示。
            </p>

            <UiCodeBlock language="vue" :code="`<script setup lang=&quot;ts&quot;>
interface Props {
  title: string
  count?: number
  user: {
