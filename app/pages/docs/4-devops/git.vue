...
<template>
    <DocsPage title="Git 版本控制" description="Git 是現代軟體開發的基石。無論是個人開發還是團隊協作，掌握 Git 都能讓你更自信地管理程式碼變更、實驗新功能以及修復錯誤。"
        :badges="[{ label: 'Version Control', color: 'primary' }, { label: 'DevOps', color: 'neutral' }]">

        <!-- Section 1: Setup -->
        <DocsSection id="setup" title="初始設定" icon="heroicons:cog-6-tooth" separator>
            <p class="text-slate-400 mb-6">
                在開始使用 Git 之前，請確保設定好你的使用者名稱與 Email，這將作為你提交紀錄的身份標識。
            </p>

            <UiCodeBlock language="bash" :code="`# 設定使用者名稱
...
git config --list`" filename="Terminal" />
        ...
        <!-- Section 2: Basics -->
        <DocsSection id="basics" title="基礎指令" icon="heroicons:command-line" separator>
            <p class="text-slate-400 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-4">
                    <div class="p-4 rounded-xl bg-slate-900/50 border border-slate-800/50">
                        ...
                        <UiCodeBlock language="bash" :code="`# 初始化專案
git init
...
git status`" filename="Terminal" />
                    ...
                    <div class="p-4 rounded-xl bg-slate-900/50 border border-slate-800/50">
                        ...
                        <UiCodeBlock language="bash" :code="`# 加入所有變更
git add .
...
git commit -m &quot;feat: initial project structure&quot;`" filename="Terminal" />
                ...
                <div class="space-y-4">
                    <div class="p-4 rounded-xl bg-slate-900/50 border border-slate-800/50">
                        ...
                        <UiCodeBlock language="bash" :code="`# 查看提交歷史
...
git diff`" filename="Terminal" />
                    ...
                    <div class="p-4 rounded-xl bg-slate-900/50 border border-slate-800/50">
                        ...
                        <UiCodeBlock language="bash" :code="`# 捨棄工作目錄的修改 (危險!)
git restore .
...
git restore --staged .`" filename="Terminal" />
        ...
        <!-- Section 3: Branching -->
        <DocsSection id="branching" title="分支操作" icon="heroicons:arrows-pointing-out" separator>
            <p class="text-slate-400 mb-6">
                分支 (Branch) 讓你可以平行開發新功能，而不影響主程式碼 (main)。
            </p>

            <UiCodeBlock language="bash" :code="`# 建立並切換到新分支
...
git branch -d feature/login-page`" filename="Terminal" />
        ...
        <!-- Section 4: Remote -->
        <DocsSection id="remote" title="遠端協作" icon="heroicons:cloud" separator>
            <p class="text-slate-400 mb-6">
                與 GitHub 或 GitLab 等遠端倉庫同步。
            </p>

            <UiCodeBlock language="bash" :code="`# 複製遠端專案
...
git pull origin main`" filename="Terminal" />
        ...
        <!-- Section 5: Advanced -->
        <DocsSection id="advanced" title="進階技巧" icon="heroicons:sparkles" separator>
            <div class="space-y-8">
                <!-- Stash -->
                <div>
                    ...
                    <UiCodeBlock language="bash" :code="`# 暫存當前修改
git stash
...
git stash pop`" filename="Terminal" />
                ...
                <div>
                    ...
                    <UiCodeBlock language="bash" :code="`# Soft Reset: 撤銷 Commit，但保留檔案修改 (回到暫存區)
...
git reset --hard HEAD~1`" filename="Terminal" />
                ...
                <div>
                    ...
                    <UiCodeBlock language="bash" :code="`# 複製特定 Commit
git cherry-pick <commit-hash>`" filename="Terminal" />
                ...
                <div>
                    ...
                    <UiCodeBlock language="bash" :code="`# 將當前分支的基底移到 main 的最新狀態
git rebase main
...
git rebase -i HEAD~3`" filename="Terminal" />
        ...
        <DocsSection id="best-practices" title="最佳實踐" icon="heroicons:check-badge" separator>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="p-5 rounded-xl bg-slate-900/50 border border-slate-800/50">
                    ...
                    <UiCodeBlock language="plaintext" :code="`node_modules/
.output/
.nuxt/
.env
dist/
