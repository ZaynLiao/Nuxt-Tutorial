...
<template>
    <DocsPage title="安全性 (Security)" description="安全性不是附加功能，而是開發的基礎。本章將介紹如何在 Nuxt 應用程式中實作輸入驗證、設定安全標頭，並保護您的敏感資料。"
        :badges="[{ label: 'Zod', color: 'primary' }, { label: 'OWASP', color: 'neutral' }]">

        <!-- Section 1: Validation -->
        <DocsSection id="validation" title="資料驗證 (Zod)" icon="heroicons:shield-check" separator>
            <p class="text-slate-400 leading-relaxed mb-6">
                永遠不要信任客戶端的輸入。我們推薦使用 <strong class="text-emerald-400">Zod</strong> 來驗證 API 請求的 Body 與 Query Parameters。
            </p>

            <UiCodeBlock language="bash" :code="`pnpm add zod h3-zod`" filename="Terminal" />

            <p class="text-slate-400 my-4">
                在 Server Handler 中使用：
            </p>

            <UiCodeBlock language="typescript" :code="`import { z } from 'zod'
...
export default defineEventHandler(async (event) => {
  ...
  return { success: true, user: body }
        ...
        <!-- Section 2: Headers -->
        <DocsSection id="headers" title="安全性標頭" icon="heroicons:lock-closed" separator>
  ...
  routeRules: {
    '/**': {
      headers: {
        ...
        <DocsSection id="env-safety" title="環境變數安全" icon="heroicons:key" separator>
            <p class="text-slate-400 mb-6">
