...
<template>
    <aside class="hidden lg:block lg:w-72 lg:shrink-0 bg-slate-950 border-r border-slate-800/50">
        <div class="sticky top-0 h-screen pt-16 overflow-y-auto px-4 py-8">
            <div class="space-y-8">
                <div v-for="group in sidebarGroups" :key="group.title" class="space-y-3">
                    <div class="px-3 text-xs font-bold tracking-widest uppercase text-slate-500">
                        {{ group.title }}
                    </div>
                    <nav class="space-y-1">
                        <NuxtLink v-for="item in group.items" :key="item.label" :to="item.to || undefined"
                            ...
                            :class="[
                                ...
                                !item.to && 'opacity-50 cursor-not-allowed hover:bg-transparent hover:text-slate-400'
                            ]">
                            <Icon :name="item.icon" class="h-5 w-5 shrink-0 transition-colors"
